...
piles_rm_anova <- aov(ellipsoid_vol ~ Location * Trip + Error(Block / Trip), data = piles2)
...
# Calculate the mean and standard error of Nitrate for each Site and Trip combination
nitrate_summary <- pile_chem %>%
  group_by(Site, Trip) %>%
  summarize(
...
pitfall_count <- ggplot(pitfalls_data) +
  ...
  geom_jitter(position = position_dodge(width = 0.5), size = 2, alpha = 0.7) +  # Jittered dots
...
theme(
  ...
  facet_grid(vars(Treatment), vars(Site), scales = "fixed", space = "fixed", labeller = labeller(Treatment = c(Control = "control", Sargassum = "sargasso"))) +
    guides(shape = FALSE)  # Hide the legend
...
combined_pitfall_graphs <- grid.arrange(
  ...
  pitfall_eff_plot,
  ncol = 1,
  heights = c(3, 2)  # Adjust the relative heights here (e.g., 3 for the top plot and 1 for the bottom plot)
)
...
stickies_prc_cover <-ggplot(stickies_data) +
  aes(x = Trip, y = mean_count, shape = Taxa) +  # Color by Treatment, shape by Site
  geom_jitter(position = position_dodge(width = 0.5), size = 2, alpha = 0.7) +  # Jittered dots
  geom_errorbar(aes(ymin = mean_count - std_error, ymax = mean_count + std_error),
                position = position_dodge(width = 0.5), width = 0.2) +  # Error bars
  theme_minimal() +
...
combined_sticky_graphs <- grid.arrange(
  ...
  stickies_eff_plot,
  ncol = 1,
  heights = c(3, 2.5)  # Adjust the relative heights here (e.g., 3 for the top plot and 1 for the bottom plot)
)
