...
perimeter_survey_long<- perimetersurvey %>%
  gather(key = "cover_type", value = "percent_cover", c(63,64))
...
interior_pc_cover <- ggplot(data = interior_survey_long,
  ...
  facet_grid(vars(), vars(Site), labeller = labeller(Site = c(Dunes = "Beach", Jungle = "Forest"))) +  # Set facet titles
  theme_minimal() +
  scale_fill_manual(values = c("grey44", "black")) +
  scale_color_manual(values = c("grey44", "black")) +
  theme(
    ...
    panel.spacing = unit(0.2, "lines"),
    text = element_text(size = 14),
    axis.title = element_text(size = 16, margin = margin(t = 0, r = 10, b = 0, l = 0)), # Adjust the margin
    axis.text = element_text(size = 14),
    axis.text.x = element_text(size = 14),
    plot.title = element_text(size = 12),
    legend.title = element_text(size = 16),  # Set the size of the legend title
    legend.text = element_text(size = 14)  # Set the size of the legend text
  ) +
  scale_shape_manual(
    values = c("circle",  "triangle"),  # Change these to the desired shapes
    labels = c("grass", "other plants")  # Change these labels
  ) +
  labs(shape = "Cover type")  # Change the legend title here
