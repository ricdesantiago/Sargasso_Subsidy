...
piles_rm_anova <- lmer(ellipsoid_vol ~ Location * Trip + (1|Block), data = piles2)
...
invert_plot <- ggplot(data = decomp_inverts,
                      aes(
                        x = inverts,
                        y = count,
                      )) +
  ...
  theme(axis.title.x = element_blank(),  # Remove x-axis title
        ...
        panel.border = element_rect(color = "black", fill = NA, size = 0.5)) +  # Add a thin black line around the graph
  labs(y = "Arthroopod counts")
...
decomp_bags %>%
  ...
  theme(
    ...
    legend.title = element_text(size = 16), 
    legend.text = element_text(size = 14),
    legend.position = "bottom"  # Add this line to move the legend to the bottom
  )
...
invert_plot <- ggplot(data = decomp_inverts,
  ...
  theme(axis.title.x = element_blank(),  # Remove x-axis title
        text = element_text(size = 16),  # Increase font size for all text
        panel.border = element_rect(color = "black", fill = NA, size = 1)) +  # Add a thin black line around the graph
  labs(y = "Arthroopod counts")
...
pile_chem %>%
  group_by(Trip, Site, Type, Trt ) %>%
  summarize(mean_Nitrate = mean(Nitrate), se = sd(Nitrate) / sqrt(n())) %>%
  ggplot() +
  aes(x = Trip, y = mean_Nitrate, fill = Type) +
...
#removed the legend from the plot but this is the code to re add it
 # "labs(
  #  x = " ",
  #  y = "gCO2/m2/h",
  #  title = "Gas collectors",
 ...
 # scale_fill_manual(
 #   values = c("lightgray", "white"),  # Define the colors
  #  labels = c("Control", "Sargasso")) +  # Define the labels for the legend
...
CO2plot2 <- respirometer %>%
  ...
  facet_grid(vars(), vars(Site), scales = "free_y") +
  scale_x_discrete(labels = c("Control", "Sargasso"))  # Update x-axis labels
...
perimeter_pc_cover<-ggplot(data = perimeter_short,
  ...
  facet_grid(vars(), vars(Site), scales = "fixed", labeller = labeller(Site = c(Dunes = "Beach", Jungle = "Forest"))) +
  theme_minimal() +
  scale_fill_manual(values = c("grey44", "black")) +
  scale_color_manual(values = c("grey44", "black")) +
  theme(
...
#########graph for beach######
beach_pitfalls_data<-subset(pitfalls_data, Site == 'Dune')

beach_pitfall_count <- ggplot(beach_pitfalls_data) +
  ...
  facet_grid(vars(Treatment), vars(), scales = "fixed", space = "fixed", labeller = labeller(Treatment = c(Control = "control", Sargassum = "sargasso"))) +
...
print(beach_pitfall_count)


#########graph for forest######
forest_pitfalls_data<-subset(pitfalls_data, Site == 'Jungle')

forest_pitfall_count <- ggplot(forest_pitfalls_data) +
  aes(x = Trip, y = mean_count, shape = Taxa) + 
  geom_jitter(position = position_dodge(width = 0.5), size = 2, alpha = 0.7) +   
  geom_errorbar(aes(ymin = mean_count - std_error, ymax = mean_count + std_error),
                position = position_dodge(width = 0.5), width = 0.2) +  
...
##############subset the beach treatment
beach_stickies_data<-subset(stickies_data, Site == 'Dune')


# Create the plot with jittered dots and error bars
stickies_beach_cover <-ggplot(beach_stickies_data) +
  aes(x = Trip, y = mean_count, shape = Taxa) +  
  geom_jitter(position = position_dodge(width = 0.5), size = 2, alpha = 0.7) +   
  geom_errorbar(aes(ymin = mean_count - std_error, ymax = mean_count + std_error),
                position = position_dodge(width = 0.5), width = 0.2) +   
...
stickies_beach_cover



##########subset forest treatment
forest_stickies_data<-subset(stickies_data, Site == 'Jungle')


# Create the plot with jittered dots and error bars
stickies_forest_cover <-ggplot(forest_stickies_data) +
  aes(x = Trip, y = mean_count, shape = Taxa) +  
  geom_jitter(position = position_dodge(width = 0.5), size = 2, alpha = 0.7) +   
  geom_errorbar(aes(ymin = mean_count - std_error, ymax = mean_count + std_error),
                position = position_dodge(width = 0.5), width = 0.2) +   
...
stickies_eff_plot <- ggplot(stickieseffsize, aes(x = Trip, y = Effect_size, shape = Order)) +
  labs(y = "Effect size", shape = "Taxa", title = "") +
  geom_path(aes(group = Order), position = position_dodge(0.5), color = "dark gray") +
  theme_bw() +
  facet_wrap(vars(Site), ncol = 2L, labeller = as_labeller(c("Dune" = "Beach", "Jungle" = "Forest"))) +
  geom_point(color = "black", size = 3, position = position_dodge(0.5)) +
...
# Assuming ladder_survey_short and ladder_survey_summed are loaded and available
