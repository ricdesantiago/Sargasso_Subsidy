...
# Fit a model of percent decomposition by treatment x site, and add a random factor to account for variability between bags.

decomp_model<- lmer(prc_Delta ~  Treatment * Site + (1|Bag_ID), data = decomp_bags_model_data, na.action = na.exclude)

isSingular(decomp_model)
...
decomp_model_full <- lmer(prc_Delta ~ Treatment * Site + (1|Bag_ID), data = decomp_bags, na.action = na.exclude)

# Perform a fully crossed post-hoc test for Treatment and Site
emmeans_results <- emmeans(decomp_model_full, pairwise ~ Treatment * Site)
...
CO2<-na.omit(CO2)
CO2$trip<-as.factor(CO2$trip)
CO2$rep<-as.factor(CO2$rep)
view(CO2)

#lets look at the data 
hist(CO2$co2)
# data are zero inflated, not normal

#let’s find out if theres the overall treatment effect (sargassum addition) on soil respiration. Here we will look to see if there is an interaction between our 3 factors, thus, we will make a three way interaction. There is no great way to look at these data with uneven  sample sizes so we will use a LMEM since its more robust and include a random factor for replicate # to account for this unevenness. 

#model
co2model <- lmer(co2 ~ treatment * site * trip + (1|rep), data = CO2)

# lets run a type 3 anova to test for overall treatment effects and interactions.
anova(co2model)
...
co2model2 <- lmer(co2 ~ treatment * site * trip + (1 + trip | rep), data = CO2)
anova(co2model2)
...
model_trip2 <- lmer(co2 ~ treatment  + (1|rep), data = trip2_data)

# Mixed-effects model for trip 2
model_trip3 <- lmer(co2 ~ treatment + (1|rep), data = trip3_data)

# Mixed-effects model for trip 3
model_trip4 <- lmer(co2 ~ treatment + (1|rep), data = trip4_data)
